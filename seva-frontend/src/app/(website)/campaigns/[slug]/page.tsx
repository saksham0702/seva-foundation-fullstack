...
function DonorPanel({ slug }: DonorPanelProps) {
  ...
  const fetchPage = useCallback(
    async (nextPage: number, direction: "left" | "right" | null = null) => {
      ...
      try {
        ...
        setLoading(false);
        // Reset slide animation after short delay
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
      ...
      {loading ? (
        ...
        <>
          ...
          >
            {donors.map((d, i) => {
              const initials = d.name === "Anonymous Donor" ? "?" : d.name[0]?.toUpperCase() || "?";
              const timeAgo = (() => {
...
export default function CampaignDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError(null);
        // Use the slug endpoint directly — stats come from DB fields
        const campaignData = await getCampaignBySlug(slug as string);
        if (!cancelled) {
          if (!campaignData) {
            setError("This campaign could not be found.");
          } else {
            setCampaign(campaignData);
          }
        }
      } catch {
        if (!cancelled) setError("Couldn't load this campaign right now.");
      } finally {
  ...
  // Use DB fields as single source of truth — updated on every successful payment
  ...
  // Determine if campaign is completed (either status field or goal fully reached)
  ...
  const productsList = Array.isArray(parsedContent.products) ? parsedContent.products : [];
  ...
  const finalAmount = activeTab === "products" ? productsTotal : Number(donateAmount || 0);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {isCompleted && (
          <div className="mb-6 flex items-center gap-3 bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 rounded-2xl px-5 py-4">
            ...
            <div>
              <p className="font-bold text-emerald-800 text-sm">🎉 Goal Achieved! This campaign has been completed.</p>
              <p className="text-emerald-600 text-xs mt-0.5">
                {fmt(currentRaised)} raised from {currentDonors} generous donor{currentDonors === 1 ? "" : "s"}.
              </p>
            </div>
          </div>
        )}
            ...
            <div id="donation-panel" className="sticky top-24 space-y-4">
              <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                <div className="flex justify-between items-baseline mb-1">
                  ...
                  {goal > 0 && (
                    <span className={`text-xs font-semibold ${isCompleted ? "text-emerald-600" : "text-[#E8542A]"}`}>
                      {pct}% of {fmt(goal)}
                    </span>
                  )}
                </div>
                ...
                {isCompleted ? (
                  ...
                  <>
                    {/* Tab selector for Money vs Products */}
                    {productsList.length > 0 && (
              ...
              {/* ── Paginated donor panel ── */}
              <DonorPanel slug={slug as string} />
            </div>
          </div>
        </div>
      </div>
